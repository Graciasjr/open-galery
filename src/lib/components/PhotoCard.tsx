...
import { Heart, MessageCircle, Share2, Smile, Download, Wand2, ThumbsUpIcon, Flame } from 'lucide-react';
...
export default function PhotoCard({ photo, onClick, onAddReaction }: PhotoCardProps) {
  ...
  const [reactx, setReactx] = useState(false)
  const [reaction, setReaction] = useState<LucideIcon | undefined>(undefined)
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: isLoaded ? 1 : 0, y: 0 }} className={`rounded-md shadow-md overflow-hidden transition-all duration-300 hover:scale-[1.02] cursor-pointer ${isLoaded ? 'opacity-100' : 'opacity-0'}`} onClick={onClick}>
      ...
      <div className="p-4">
        {reactx && <div className='flex text-yellow-500 absolute -mt-16 bg-white shadow-lg mb-4 px-6 py-4 space-x-4 border w-max rounded-3xl'>
          <motion.button
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            onClick={() => {
              setReaction(react.smile);
              setReactx(false)
            }}
            className="flex items-center space-x-1 text-gray-500 hover:text-red-500 transition-colors"
          >
            {<react.smile size={18} className="transition-transform hover:scale-110" />}
          </motion.button>
          <motion.button
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            onClick={() => {
              setReaction(react.love);
              setReactx(false)
            }}
            className="flex items-center space-x-1 text-gray-500 hover:text-red-500 transition-colors"
          >
            {<react.love size={18} className="transition-transform hover:scale-110" />}
          </motion.button>
          <motion.button
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            onClick={() => {
              setReaction(react.like);
              setReactx(false)
            }}
            className="flex items-center space-x-1 text-gray-500 hover:text-red-500 transition-colors"
          >
            {<react.like size={18} className="transition-transform hover:scale-110" />}
          </motion.button>
          <motion.button
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            onClick={() => {
              setReaction(react.fire);
              setReactx(false)
            }}
            className="flex items-center space-x-1 text-gray-500 hover:text-red-500 transition-colors"
          >
            {<react.fire size={18} className="transition-transform hover:scale-110" />}
          </motion.button>
        </div>}
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="relative">
              <motion.button onClick={
                ...
                {reaction ? React.createElement(reaction, { size: 18 }) : <Smile size={18} />}
